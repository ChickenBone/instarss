...
def _run_feeds_job(config: Config, db_conn: sqlite3.Connection, client: InstapaperClient) -> None:
...
def _process_single_feed(
    ...
    for item in items:
        ...
        try:
            ...
            logger.critical("Instapaper auth error for feed [%s] — check credentials", feed.name)
            raise
        except Exception as exc:
